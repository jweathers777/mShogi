////////////////////////////////////////////////////////////////////////////
// Name:         SoaringEagle.cpp
// Description:  Implementation for a class that represents a soaring eagle
// Created:      09/02/2004 11:02:01 Eastern Daylight Time
...
//       Class:  SoaringEagle
//      Method:  SoaringEagle
// Description:  Constructs an instance of a soaring eagle
//--------------------------------------------------------------------------
SoaringEagle::SoaringEagle(Board* board, int value, int typevalue)
{
   mpBoard = board; 
   mValue = value;
   ...
   mNotation = "SE";
   mNames[0] = "Soaring Eagle"; mNames[1] = "Hiju";
   mDescription =
      "Moves any number of squares in any direction except forward diagonally or like a Lion forward diagonally";
...
//------------------------------------------------------------------------
void SoaringEagle::InitStandardAttackPatterns()
{
...
//--------------------------------------------------------------------------
bool SoaringEagle::ThreatensSquare(Piece* piece, int square)
{
   ...
   // Can we make a single step forward northwest to threaten our target?
   ...
   if (finish != Board::OFF_BOARD) {
      ...
      finish = mpBoard->NorthWest(color, finish);
      if (finish == square) return true;
   }
 
   // Can we make a single step forward northeast to threaten our target?
   finish = mpBoard->NorthEast(color, start);
   if (finish != Board::OFF_BOARD) {
      if (finish == square) return true;
      
      // Can we make a two step move forward to threaten our target?
      finish = mpBoard->NorthEast(color, finish);
      if (finish == square) return true;
   }
...
//--------------------------------------------------------------------------
void SoaringEagle::GenerateCaptures(std::vector<Move>& movelist, 
                                          Piece* piece)
{
   ...
   // First calculate lion attack captures in the northwest direction
   finish = mpBoard->NorthWest(color, start);
   if (finish != Board::OFF_BOARD) {
      captured = mpBoard->mSquares[finish];
      captureA = captured && (captured->mColor == enemy);
      finish2 = mpBoard->NorthWest(color, finish);
   ...
   captureA = false;
   captureB = false;
   // Second calculate lion attack captures in the northeast direction
   finish = mpBoard->NorthEast(color, start);
   if (finish != Board::OFF_BOARD) {
      captured = mpBoard->mSquares[finish];
      captureA = captured && (captured->mColor == enemy);
      finish2 = mpBoard->NorthEast(color, finish);
      if (finish2 != Board::OFF_BOARD) {
         captured2 = mpBoard->mSquares[finish2];
         captureB = captured2 && (captured2->mColor == enemy);
      }
...
//--------------------------------------------------------------------------
void SoaringEagle::GenerateNonCaptures(std::vector<Move>& movelist, 
                                             Piece* piece)
{
   ...
   // Test whether we can move forward in the northwest direction
   finish = mpBoard->NorthWest(color, start);
   if (finish != Board::OFF_BOARD) {
   ...
   // Test whether we can move forward in the northeast direction
   finish = mpBoard->NorthEast(color, start);
   if (finish != Board::OFF_BOARD) {
      // Check whether or not there is a piece in front of us
      if (!mpBoard->mSquares[finish]) {
